...
# 上传落地目录：
# - 本地开发：默认写入同级 backend/uploads（与 FastAPI 静态目录一致）
# - 独立/云托管部署：通过环境变量 UPLOAD_DIR 指定容器内目录（建议挂载持久卷或对象存储）
PROJECT_ROOT = Path(__file__).resolve().parents[2]
_DEFAULT_UPLOAD = PROJECT_ROOT / "backend" / "uploads"
UPLOAD_ROOT = Path(os.getenv("UPLOAD_DIR", str(_DEFAULT_UPLOAD)))
