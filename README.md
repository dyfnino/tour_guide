...
后台管理使用 Streamlit 快速搭建，与 FastAPI 共享同一 MySQL 数据库（独立的同步连接，互不干扰）。
...
- 默认账号：`admin`
- 默认密码：`admin123`
- 仅默认管理员账号可登录；可通过环境变量覆盖（写在 `backend/.env` 或系统环境变量都可）：
...
| 模块     | 功能                                                                  |
| -------- | --------------------------------------------------------------------- |
| 控制台   | 用户/课程/订单/题目/直播/回放/AI测评 等核心数据指标一览               |
| 课程管理 | 视频/音频媒体地址、课时、价格、分类、上下架                           |
| 题库管理 | 单选/多选/判断 增删改查、答案、解析                                   |
| 商品管理 | 价格/原价/库存/分类/新品/热销                                         |
| 直播管理 | 场次创建、流地址、状态切换（变 `ended` 时自动生成同名"（回放）"记录） |
| 回放管理 | 回放视频地址、时长、观看数                                            |
| AI 测评  | 测评类型管理、历史结果查看与删除                                      |
| 订单管理 | 订单查看、订单项明细、状态变更（不允许直接改总金额）                  |
| 用户管理 | 基本信息编辑、启停账号、设置管理员、重置密码（自动 hash）             |

### 注意

- 后台所有变更对小程序立即生效（共享同一 MySQL）。
- 媒体文件（视频/音频/封面）请放在自有 CDN（OSS/COS/七牛），并在小程序后台配置合法域名。
- 订单总金额不允许在后台直接修改，仅能切换状态（unpaid/paid/completed）；如需调金额请走业务退款流程。
- 用户表不允许修改 `username`，避免与登录态冲突。
